    ...
    where T : Entity
{
    ...
    public void InsertRange(IEnumerable<T> models)
    {
        ...
        dbContext.Set<T>().AddRange(models.Where(m => m != null));
    }
    ...
    // Raw SQL
    // -------------------------
    public async Task<int?> ExecuteQueryAsync(string sql, CancellationToken ct = default)
    {
    ...
    {
        return await GetNoTrackingConditional(predicate)
